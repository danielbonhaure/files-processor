...
class ReadCPToutput(ReadStrategy):
    """
    ...
    def read_data(self, file_name: str) -> DataFrame:
        ...
        data_df = pd.read_csv(file_name, sep='\t', names=header_df.columns.to_list(), index_col=0,
                              skiprows=info.data_first_line, nrows=info.data_last_line, na_values=info.na_values)
        ...
        if file_config.get('swap_years', False):
            ...
            # Se reemplaza los valores con NA en final_df con los valores coregidos
        ...
        with open(file_name) as fp:
            ...
            for cnt, line in enumerate(fp):
                ...
                if n_rows_regex and n_cols_regex and na_values_regex:
                    ...
                    header_line, data_first_line, data_last_line = cnt + 1, cnt + 4, n_rows
                    # Retornar los datos del archivo
                    return CPToutputFileInfo(n_rows, n_cols, na_values, header_line, data_first_line, data_last_line)
...
class ReadEregDEToutput(ReadStrategy):
    """
    A Concrete Strategy (Desing Pattern -> Strategy)
    """
    def read_data(self, file_name: str) -> DataFrame:
        ...
        # Crear dataframe a partir del dataset creado antes
...
class ReadEregPROBoutput(ReadStrategy):
    """
    A Concrete Strategy (Desing Pattern -> Strategy)
    """
    def read_data(self, file_name: str) -> DataFrame:
